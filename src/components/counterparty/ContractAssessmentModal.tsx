...
export const RISKS: ContractRisk[] = [
  {
    ...
    title: "Отказ Заказчика компенсировать расходы по возврату товаров",
    description:
      "При возврате бракованного товара расходы первоначально несет Купер, а партнер компенсирует эти расходы только через несколько дней, что создает риск кассового разрыва, а отсутствие механизма автоматического удержания этих сумм из текущего вознаграждения создает риск невозврата средств.",
  },
  {
    id: "m-1",
    level: "medium",
    category: "Правовые риски",
    title: "Предъявление исков третьими лицами о нарушении прав",
    description:
      "Если контент или товары партнера нарушат чужие права, Купер (Инстамарт Сервис) заплатит штрафы и судебные издержки, а потом возникнет регрессный иск к партнеру.",
  },
  {
    id: "m-2",
    level: "medium",
    category: "Правовые риски",
    title: "Расторжение договора Исполнителем в одностороннем порядке",
    description:
      "Партнер может в любой момент разорвать договор без суда и не возместить Купер (Инстамарт Сервис) убытки от внезапной остановки продаж, даже если причина разрыва спорная.",
  },
  {
    id: "m-3",
    level: "medium",
    category: "Процессные риски",
    title: "Поступление недостоверных данных о ценах и остатках",
    description:
      "Цена на кассе может отличаться от цены на сайте. Вместо блокировки продажи или автоматического аннулирования заказа договор предусматривает лишь последующий финансовый перерасчет между сторонами. Это подтверждает отсутствие механизма оперативной реакции на расхождение данных в момент покупки, что ведет к потенциальным конфликтам с покупателями.",
  },
];
